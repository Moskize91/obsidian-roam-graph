...
- `src/lib/canvas.ts` 放 Canvas JSON 类型和布局生成逻辑。当前布局是中心节点加环形邻居。
...
插件监听 Obsidian 的 `workspace.on("file-open")`。当 active file 是 Markdown 文件时，插件会 debounce 一次刷新：
...
插件也监听 `metadataCache.on("resolved")`，在链接解析完成后刷新最近的中心节点。
